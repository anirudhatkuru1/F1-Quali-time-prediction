...
.app-nav-left { font-size: 15px; font-weight: 600; color: #1d1d1f; letter-spacing: -0.01em; }
.app-nav-pills {
  display: flex; gap: 2px; background: #f5f5f7;
...
.dc-time { font-size: 34px; font-weight: 600; letter-spacing: -0.03em;
  color: #1d1d1f; font-variant-numeric: tabular-nums; margin-bottom: 4px; }
.dc-gap { font-size: 14px; color: #86868b; margin-bottom: 18px; }
.dc-row { display: flex; gap: 24px; }
.dc-stat-l { font-size: 11px; color: #86868b; margin-bottom: 3px; }
.dc-stat-v { font-size: 14px; font-weight: 600; color: #1d1d1f; }

/* ── Gap badge ── */
.gap-badge {
  background: #ffffff; border: 0.5px solid rgba(0,0,0,0.08);
  border-radius: 14px; padding: 13px 20px;
  font-size: 14px; color: #6e6e73; text-align: center; margin-bottom: 28px;
}
.gap-badge b { color: #1d1d1f; }
...
.g-pos { font-size: 13px; font-weight: 500; color: #86868b; }
.g-dot { width: 8px; height: 8px; border-radius: 50%; }
.g-name { font-size: 14px; font-weight: 600; color: #1d1d1f; letter-spacing: -0.01em; }
.g-bar-w { height: 3px; background: #f5f5f7; border-radius: 2px; }
.g-bar   { height: 3px; border-radius: 2px; min-width: 2px; }
.g-gap { font-size: 13px; color: #6e6e73; text-align: right; font-variant-numeric: tabular-nums; }
.g-gap.pole { color: #1d1d1f; font-weight: 600; }
...
.mc-l { font-size: 12px; color: #6e6e73; margin-bottom: 4px; }
.mc-v { font-size: 24px; font-weight: 700; letter-spacing: -0.025em; color: #1d1d1f; }
.mc-s { font-size: 12px; color: #86868b; margin-top: 2px; }
...
.cc-t { font-size: 13px; font-weight: 600; color: #1d1d1f; margin-bottom: 2px; }
.cc-s { font-size: 12px; color: #6e6e73; margin-bottom: 12px; }
...
.ic-t { font-size: 14px; font-weight: 600; color: #1d1d1f; margin-bottom: 8px; }
.ic-b { font-size: 14px; color: #6e6e73; line-height: 1.6; }
.ic-hl { color: #1d1d1f; font-weight: 600; }
...
.rt-hdr { background: #f5f5f7; font-size: 11px; font-weight: 600;
  letter-spacing: 0.04em; text-transform: uppercase; color: #86868b; }
.rg { color: #34c759; } .rb { color: #ff3b30; }

/* ── Section title ── */
.pg-title { font-size: 36px; font-weight: 700; letter-spacing: -0.04em;
  color: #1d1d1f; margin: 44px 0 4px; }
.pg-sub { font-size: 15px; color: #6e6e73; margin: 0 0 24px; }

/* ── Sub nav ── */
.sub-nav {
  display: flex; border-bottom: 0.5px solid rgba(0,0,0,0.09);
  margin-bottom: 28px;
}
.sn-item {
  font-size: 14px; color: #6e6e73; padding: 10px 0;
  margin-right: 28px; border-bottom: 2px solid transparent;
  transition: all 0.15s; font-weight: 400;
}
.sn-item.on { color: #1d1d1f; font-weight: 600; border-bottom-color: #1d1d1f; }
...
# This is the ONLY reliable way to get clicks from a component back to Python
qp = st.query_params
if "team" in qp:
    t = urllib.parse.unquote(qp["team"])
    if t in ["Red Bull Racing", "McLaren", "Ferrari", "Mercedes", "Aston Martin", "Alpine", "Williams", "Haas F1 Team", "RB", "Kick Sauber"]:
...
.nav-brand{font-size:13px;font-weight:600;color:#1d1d1f;letter-spacing:0.01em}
.nav-year{font-size:12px;color:#86868b;letter-spacing:0.06em;text-transform:uppercase}
...
.title-f1{
  ...
  display:block;
  font-family:-apple-system,BlinkMacSystemFont,"Helvetica Neue",sans-serif;
  /* Bernoru Semi Condensed fallback — closest available */
  font-stretch:condensed;
}
/* Italic script: "Prediction." */
.title-pred{
...
<body>
<div class="hero">
  <img class="car-img" src="data:image/png;base64,""" + CAR_B64 + """" alt="F1 Car"/>
  <div class="overlay"></div>
  <div class="nav">
    <span class="nav-brand">F1 Qualifying Predictor</span>
    <span class="nav-year">2025 Season</span>
  </div>
  <div class="content">
    <span class="title-f1">F1 Qualifying</span>
    <span class="title-pred">Prediction.</span>
  </div>
  <div class="bottom">
    <span class="credit">Done by Anirudh Atkuru</span>
    <a class="cta" onclick="go()" href="#">
      <span class="cta-label">Select your team</span>
...
<script>
function go(){
  // Set query param — Streamlit reads this and reruns
  window.parent.location.href = window.parent.location.pathname + "?nav=selector";
}
</script>
</body>
</html>"""
...
# ════════════════════════════════════════════════════════════════════════════
# SCREEN 2: TEAM SELECTOR — liquid glass dark grid with driver faces
# ════════════════════════════════════════════════════════════════════════════
if st.session_state.screen == "selector":
    st.markdown("""<style>
    .stApp,[data-testid="stAppViewContainer"],[data-testid="stMain"],
    .main,.block-container,iframe{background:#0a0a0a !important;padding:0 !important;border:none !important}
    </style>""", unsafe_allow_html=True)

    grid_json = "[{\"team\": \"Red Bull Racing\", \"d1\": \"VER\", \"d2\": \"LAW\", \"tc\": \"#3671C6\", \"r1\": \"\", \"r2\": \"\", \"img1\": \"https://upload.wikimedia.org/wikipedia/commons/thumb/6/60/Max_Verstappen_2023_British_Grand_Prix_%28cropped%29.jpg/200px-Max_Verstappen_2023_British_Grand_Prix_%28cropped%29.jpg\", \"img2\": \"https://upload.wikimedia.org/wikipedia/commons/thumb/0/04/Liam_Lawson%2C_2024_Singapore_GP.jpg/200px-Liam_Lawson%2C_2024_Singapore_GP.jpg\"}, {\"team\": \"McLaren\", \"d1\": \"NOR\", \"d2\": \"PIA\", \"tc\": \"#FF8000\", \"r1\": \"\", \"r2\": \"\", \"img1\": \"https://upload.wikimedia.org/wikipedia/commons/thumb/f/f5/Lando_Norris_2023_British_Grand_Prix.jpg/200px-Lando_Norris_2023_British_Grand_Prix.jpg\", \"img2\": \"https://upload.wikimedia.org/wikipedia/commons/thumb/3/38/Oscar_Piastri_2023_British_Grand_Prix_%28cropped%29.jpg/200px-Oscar_Piastri_2023_British_Grand_Prix_%28cropped%29.jpg\"}, {\"team\": \"Ferrari\", \"d1\": \"LEC\", \"d2\": \"HAM\", \"tc\": \"#E8002D\", \"r1\": \"\", \"r2\": \"\", \"img1\": \"https://upload.wikimedia.org/wikipedia/commons/thumb/f/fe/Charles_Leclerc_2023_British_Grand_Prix_%28cropped%29.jpg/200px-Charles_Leclerc_2023_British_Grand_Prix_%28cropped%29.jpg\", \"img2\": \"https://upload.wikimedia.org/wikipedia/commons/thumb/1/18/Lewis_Hamilton_2016_Malaysia_2_%28cropped%29.jpg/200px-Lewis_Hamilton_2016_Malaysia_2_%28cropped%29.jpg\"}, {\"team\": \"Mercedes\", \"d1\": \"RUS\", \"d2\": \"ANT\", \"tc\": \"#27F4D2\", \"r1\": \"\", \"r2\": \"R\", \"img1\": \"https://upload.wikimedia.org/wikipedia/commons/thumb/a/a9/George_Russell_2023_British_Grand_Prix_%28cropped%29.jpg/200px-George_Russell_2023_British_Grand_Prix_%28cropped%29.jpg\", \"img2\": \"https://upload.wikimedia.org/wikipedia/commons/thumb/6/64/Kimi_Antonelli_2024_Italian_Grand_Prix_%28cropped%29.jpg/200px-Kimi_Antonelli_2024_Italian_Grand_Prix_%28cropped%29.jpg\"}, {\"team\": \"Aston Martin\", \"d1\": \"ALO\", \"d2\": \"STR\", \"tc\": \"#229971\", \"r1\": \"\", \"r2\": \"\", \"img1\": \"https://upload.wikimedia.org/wikipedia/commons/thumb/b/ba/Fernando_Alonso_2023_British_Grand_Prix_%28cropped%29.jpg/200px-Fernando_Alonso_2023_British_Grand_Prix_%28cropped%29.jpg\", \"img2\": \"https://upload.wikimedia.org/wikipedia/commons/thumb/5/5b/Lance_Stroll_2023_British_Grand_Prix_%28cropped%29.jpg/200px-Lance_Stroll_2023_British_Grand_Prix_%28cropped%29.jpg\"}, {\"team\": \"Alpine\", \"d1\": \"GAS\", \"d2\": \"DOO\", \"tc\": \"#FF87BC\", \"r1\": \"\", \"r2\": \"\", \"img1\": \"https://upload.wikimedia.org/wikipedia/commons/thumb/8/8f/Pierre_Gasly_2023_British_Grand_Prix_%28cropped%29.jpg/200px-Pierre_Gasly_2023_British_Grand_Prix_%28cropped%29.jpg\", \"img2\": \"https://upload.wikimedia.org/wikipedia/commons/thumb/e/e3/Jack_Doohan_2023_Macau_Grand_Prix_%28cropped%29.jpg/200px-Jack_Doohan_2023_Macau_Grand_Prix_%28cropped%29.jpg\"}, {\"team\": \"Williams\", \"d1\": \"ALB\", \"d2\": \"SAI\", \"tc\": \"#64C4FF\", \"r1\": \"\", \"r2\": \"\", \"img1\": \"https://upload.wikimedia.org/wikipedia/commons/thumb/d/d6/Alexander_Albon_2023_British_Grand_Prix_%28cropped%29.jpg/200px-Alexander_Albon_2023_British_Grand_Prix_%28cropped%29.jpg\", \"img2\": \"https://upload.wikimedia.org/wikipedia/commons/thumb/2/29/Carlos_Sainz_Jr_2023_British_Grand_Prix_%28cropped%29.jpg/200px-Carlos_Sainz_Jr_2023_British_Grand_Prix_%28cropped%29.jpg\"}, {\"team\": \"Haas F1 Team\", \"d1\": \"BEA\", \"d2\": \"OCO\", \"tc\": \"#B6BABD\", \"r1\": \"\", \"r2\": \"\", \"img1\": \"https://upload.wikimedia.org/wikipedia/commons/thumb/0/04/Oliver_Bearman_2024_British_Grand_Prix_%28cropped%29.jpg/200px-Oliver_Bearman_2024_British_Grand_Prix_%28cropped%29.jpg\", \"img2\": \"https://upload.wikimedia.org/wikipedia/commons/thumb/5/52/Esteban_Ocon_2023_British_Grand_Prix_%28cropped%29.jpg/200px-Esteban_Ocon_2023_British_Grand_Prix_%28cropped%29.jpg\"}, {\"team\": \"RB\", \"d1\": \"TSU\", \"d2\": \"HAD\", \"tc\": \"#6692FF\", \"r1\": \"\", \"r2\": \"R\", \"img1\": \"https://upload.wikimedia.org/wikipedia/commons/thumb/c/c8/Yuki_Tsunoda_2023_British_Grand_Prix_%28cropped%29.jpg/200px-Yuki_Tsunoda_2023_British_Grand_Prix_%28cropped%29.jpg\", \"img2\": \"https://upload.wikimedia.org/wikipedia/commons/thumb/8/8a/Isack_Hadjar_2024_Silverstone_%28cropped%29.jpg/200px-Isack_Hadjar_2024_Silverstone_%28cropped%29.jpg\"}, {\"team\": \"Kick Sauber\", \"d1\": \"HUL\", \"d2\": \"BOR\", \"tc\": \"#52E252\", \"r1\": \"\", \"r2\": \"R\", \"img1\": \"https://upload.wikimedia.org/wikipedia/commons/thumb/4/48/Nico_H%C3%BClkenberg_2023_British_Grand_Prix_%28cropped%29.jpg/200px-Nico_H%C3%BClkenberg_2023_British_Grand_Prix_%28cropped%29.jpg\", \"img2\": \"https://upload.wikimedia.org/wikipedia/commons/thumb/5/5f/Gabriel_Bortoleto_2024_Zandvoort_%28cropped%29.jpg/200px-Gabriel_Bortoleto_2024_Zandvoort_%28cropped%29.jpg\"}]"
...
*{margin:0;padding:0;box-sizing:border-box}
html,body{background:#0a0a0a;color:#fff;font-family:-apple-system,BlinkMacSystemFont,"Helvetica Neue",sans-serif;}
...
.back{font-size:13px;font-weight:500;color:#0071e3;cursor:pointer;letter-spacing:0.01em}
.back:hover{opacity:0.75}
.bar-title{font-size:11px;font-weight:700;letter-spacing:0.12em;text-transform:uppercase;color:#86868b}
.heading{padding:44px 40px 28px}
.heading h1{
...
.heading p{font-size:15px;color:rgba(255,255,255,0.35);}
.grid{
...
.accent{height:3px;width:100%;position:relative;z-index:2}

/* Card body */
.card-body{padding:20px 24px 22px;position:relative;z-index:2}
.team-label{
...
/* Driver row: photo + name side by side */
.driver-row{
...
.driver-photo{
  width:100%;aspect-ratio:1/1.1;object-fit:cover;object-position:top;
  border-radius:12px;
  filter:grayscale(20%);
  background:rgba(255,255,255,0.05);
...
.d1-name{color:#ffffff;}
.d2-name{color:rgba(255,255,255,0.4);}
.rookie-badge{
  font-size:9px;font-weight:700;letter-spacing:0.08em;
  color:rgba(255,200,0,0.7);vertical-align:super;margin-left:1px;
}
</style>
</head>
<body>
<div class="page">
  <div class="topbar">
    ...
    <span class="bar-title">2025 Driver Line-Up</span>
  </div>
  <div class="heading">
    <h1>Choose a Team.</h1>
    <p>Select the team you want to predict and analyse.</p>
  </div>
  <div class="grid" id="grid"></div>
</div>
<script>
const teams = """ + grid_json + """;
const grid = document.getElementById('grid');

teams.forEach(t => {
  const card = document.createElement('div');
  card.className = 'card';
  card.onclick = () => selectTeam(t.team);

  const img1 = t.img1
    ? `<img class="driver-photo" src="${t.img1}" alt="${t.d1}" onerror="this.style.display='none'">`
    : `<div class="driver-photo no-img">?</div>`;
  const img2 = t.img2
    ? `<img class="driver-photo" src="${t.img2}" alt="${t.d2}" onerror="this.style.display='none'">`
    : `<div class="driver-photo no-img">?</div>`;

  card.innerHTML = `
    <div class="accent" style="background:${t.tc}"></div>
    <div class="card-body">
      <div class="team-label">${t.team}</div>
      <div class="driver-row">
        <div class="driver-slot">
          ${img1}
          <div class="driver-name d1-name">${t.d1}<span class="rookie-badge">${t.r1}</span></div>
        </div>
        <div class="driver-slot">
          ${img2}
          <div class="driver-name d2-name">${t.d2}<span class="rookie-badge">${t.r2}</span></div>
        </div>
      </div>
    </div>`;
...
function selectTeam(name) {
  // Navigate parent to set query param — Streamlit reads and reruns
  const encoded = encodeURIComponent(name);
  window.parent.location.href = window.parent.location.pathname + '?team=' + encoded;
}
function goBack() {
  window.parent.location.href = window.parent.location.pathname + '?nav=hero';
}
</script>
</body>
</html>"""

    components.html(selector_html, height=1100, scrolling=True)
    st.stop()
...
if page == "predict":
    ...
    st.markdown("<div class='g-label'>Predicted grid — Q3 · SOFT · historical weather</div>", unsafe_allow_html=True)
    rows_h = ""
    ...
    if not real_race.empty:
        ...
        for drv,pred_a in [(drv1,abs1),(drv2,abs2)]:
            ...
            if not rr.empty:
                ...
                ec="rg" if abs(err)<=0.5 else ("rb" if abs(err)>1.5 else "")
                clr=d1c if drv==drv1 else d2c
                rh += f"""<div class='rt-row'>
                  <span style='font-size:14px;font-weight:600;color:{clr}'>{drv}</span>
                  <span style='color:#1d1d1f'>{secs_to_str(rt)}</span>
                  <span style='color:#6e6e73'>{secs_to_str(pred_a)}</span>
